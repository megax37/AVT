...
void createSphere(float radius, int divisions, MyMesh *mesh, int objId) {
	...
	computeVAO(divisions + 1, p + 2, p, divisions * 2, 0.0f, mesh, objId);
}
...
void createTorus(float innerRadius, float outerRadius, int rings, int sides, MyMesh *mesh, int objId) {
	...
	computeVAO(sides + 1, p + 2, p, rings, 0.0f, mesh, objId);
}
...
void createCylinder(float height, float radius, int sides, MyMesh *mesh, int objId) {
	...
	computeVAO(4, p + 2, p, sides, 0.0f, mesh, objId);
}
...
void createPawn(MyMesh *mesh, int objId) {
	...
	computeVAO(numP, p, points, sides, smoothCos, mesh, objId);
}

void computeVAO(int numP, float *p, float *points, int sides, float smoothCos, MyMesh *mesh, int objId) {
